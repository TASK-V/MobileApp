...
export default class LPReqDetailsforLR extends Component {

    static navigationOptions = {
        title: 'Loan Providers \'s Request details',
        headerStyle: {
    ...
    loadData() {
       // //console.warn("this is inside the load function ")
        ...
        const url = 'http://192.168.8.103:8080/api/auth/getuserareq'
        fetch(url, {
                method: 'GET',
                headers: {
...
toggleModal2 = () => {
 ...
 stuatusput = (id,uname,nego) => {
    ...
    const {userbusername} = this.state;
...
//suname - LP, runame - LR
   ...
   var data ={"reqid":uid,"runame":userbusername,"suname":uname, "receiverstatus":"Accepted","senderstatus":nego,"lawyer":"notyest","comments": "commentNo"}
                        ...
                        <Left>
                          ...
                          <Text style={{color:'#4824F6'}}>RequestID:</Text>
                           ...
                           <Text style={{color:'black'}}>{item.rate} </Text> 
                            ...
                            <Text style={{color:'black'}}>Loan Payable   =   Total Amount  +   Total Interest </Text> 
                           <Text style={{color:'#4824F6'}}>{item.amount+((item.amount/100)*item.rate)} =   {item.amount}  + {(item.amount/100)*item.rate}       </Text> 

                            <Button
                                  title={'Accept'}
                                 style={styles.input}
                                //onPress={ this.DeleteUser.bind(item.uname)}
                               // onPress={ this.stuatusput.bind(item.id)}
                                onPress={() => this.stuatusput(item.id,item.uname,item.negotiation)}
                                        />
